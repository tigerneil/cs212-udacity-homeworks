...
def bowling(balls):
    ...
    for ball in balls:
        if prev_ball == -1:
            ...
            elif ball == 10 and frame < 10:
                frames[frame] = list([ball])
                frame += 1
            elif ball == 10 and frame == 10:
                prev_ball = ball
                frame_balls.append(ball)
    ...
    for i in xrange(1, 11):
        balls = frames[i]
        if i == 10:
            first_two_balls = balls[0:2]
            score += sum(first_two_balls) + bonus[0]*balls[0] + bonus[1]*balls[1] + prev_bonus*balls[0]
            if len(balls) == 3:
                score += balls[2]
        else:
            balls_sum = sum(balls)
            frame_score = 0
            if balls_sum == 10 and len(balls) == 2:
                #spare
                frame_score = 10 + bonus[0]*balls[0] + bonus[1]*balls[1] + prev_bonus*balls[0]
                bonus = (1, 0)
                prev_bonus = 0
            elif balls_sum == 10 and len(balls) == 1:
                #strike
                frame_score = 10 + bonus[0]*10 + prev_bonus*10
                if bonus == (1, 1):
                    prev_bonus = 1
                bonus = (1, 1)
            else:
                frame_score = balls_sum + bonus[0]*balls[0] + bonus[1]*balls[1] + prev_bonus*balls[0]
                bonus = (0, 0)
                prev_bonus = 0
            score += frame_score
