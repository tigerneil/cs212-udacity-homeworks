...
def solve_parking_puzzle(start, N=N):
    ...
    def successors(state):
        ...
        for pair in state:
            if is_car(pair):
                possible_moves = moves(pair, state, N)
                for move in possible_moves:
                    new_pair_location = list(pair[1])
                    new_pair_location = map(lambda x: x+move, new_pair_location)
                    result[copy_state(state, (pair[0], tuple(new_pair_location)))] = (pair[0], move)
        return result
    ...
    def is_goal(state):
        ...
        target_car_pair = next( pair
              for pair in state
              if pair[0] == '*'
             )
        return exit_pair[1][0] in target_car_pair[1]
    
    return shortest_path_search(start, successors, is_goal)

def is_car(pair):
    return pair[0] != '@' and pair[0] != '|'

def moves(pair, state, N):
    if is_vertical(pair, N) is True:
        return collect_one_way_moves(pair, N, state) + collect_one_way_moves(pair, -N, state)
    else:
        return collect_one_way_moves(pair, 1, state) + collect_one_way_moves(pair, -1, state)

def is_vertical(pair, N):
    return abs(pair[1][0] - pair[1][1]) > 1

def collect_one_way_moves(pair, move_inc, state):
    move = move_inc
    possible_moves = []
    pair = (pair[0], map(lambda x: x+move_inc, pair[1]))
    while location_possible(pair, state):
        possible_moves.append(move)
        move += move_inc
        pair = (pair[0], map(lambda x: x+move_inc, pair[1]))
    return possible_moves

def location_possible(pair, state):
    for p in state:
        if p[0] == pair[0] or p[0] == '@': continue
        if len(set(p[1]) & set(pair[1])) > 0:
            return False
    return True    

def copy_state(state, pair):
    new_state = []
    for p in state:
        if p[0] == pair[0]:
...
def grid(cars, N=N):
    ...
    for car in cars:
        grid.append(car)
    middle = int(math.floor(N**2/2)) - 1
    grid.append(('@', (middle,)))
    wall_location = []
    for i in xrange(N-1):
        wall_location.append(i)
    for i in xrange(N-1, N**2 - N, N):
        if i != middle:
            wall_location.append(i)
        if i+1 != middle:
            wall_location.append(i+1)
    for i in xrange(N**2 - N + 1, N**2):
        wall_location.append(i)
    grid.append(('|', tuple(wall_location)))
    return tuple(grid)
