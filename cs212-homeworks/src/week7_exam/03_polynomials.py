...
def poly(coefs):
    ...
    formula_components = []
    formula_name_components = []
    for power, coef in enumerate(coefs):
        formula_components.append(str(coef) + " * x**" + str(power))
        name_component = ""
        if coef > 1 or (coef > 0 and power == 0):
            name_component += str(coef)
        if coef > 0 and power > 0:
            if coef > 1:
                name_component += " * "
            name_component += "x"
        if coef > 0 and power > 1:
            name_component += "**"+str(power)
        if len(name_component) > 0:
            formula_name_components.append(name_component)
    formula_components.reverse()
    formula_name_components.reverse()
    def p1 (x):
        return eval(' + '.join(formula_components))
    p1.coefs = coefs
    p1.__name__ = ' + '.join(formula_name_components)
    return p1
...
def add(p1, p2):
    ...
    def add_coeffs(x=0, y=0): return x+y
    coefs1, coefs2 = sum_normilize_polynomials(p1, p2)
    new_coefs = map(add_coeffs, coefs1, coefs2)
    return poly(tuple(new_coefs));

def sub(p1, p2):
    "Return a new polynomial which is the difference of polynomials p1 and p2."
    def sub_coeffs(x=0, y=0): return x-y
    coefs1, coefs2 = sum_normilize_polynomials(p1, p2)
    new_coefs = map(sub_coeffs, coefs1, coefs2)
    return poly(tuple(new_coefs));

def mul(p1, p2):
    "Return a new polynomial which is the product of polynomials p1 and p2."
    max_power = len(p1.coefs) + len(p2.coefs) - 2
    coefs1, coefs2 = mul_normilize_polynomials(p1, p2)
    new_coefs = []
    for k in xrange(max_power+1):
        coeff = 0
        for i in xrange(k+1):
            coeff += coefs1[i]*coefs2[k-i]
        new_coefs.append(coeff)
    return poly(tuple(new_coefs))
...
def mul_normilize_polynomials(p1, p2):
    coefs1 = p1.coefs
    coefs2 = p2.coefs
    max_power = len(coefs1) + len(coefs2) - 2
    coefs1 = list(coefs1) + list([0]*((max_power+1)-len(coefs1)))
    coefs2 = list(coefs2) + list([0]*((max_power+1)-len(coefs2)))
    return (tuple(coefs1), tuple(coefs2))

def sum_normilize_polynomials(p1, p2):
    coefs1 = p1.coefs
    coefs2 = p2.coefs
    max_len = max([len(coefs1), len(coefs2)])
    coefs1 = list(coefs1) + list([0]*(max_len-len(coefs1)))
    coefs2 = list(coefs2) + list([0]*(max_len-len(coefs2)))
    return (tuple(coefs1), tuple(coefs2))
...
def deriv(p):
    ...
    new_coefs = []
    for power, coef in enumerate(p.coefs):
        if power > 0:
