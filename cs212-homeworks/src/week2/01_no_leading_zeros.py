...
def compile_formula(formula, verbose=False):
    ...
    first_letters = []
    for token in tokens:
        looking_for_letters = re.findall('([A-Z])', token)
        if len(looking_for_letters) > 1:
            first_letters.append(looking_for_letters[len(looking_for_letters) - 1])
    first_letters = set(first_letters)
    body = ''.join(tokens)
    condition = ''
    i = 0
    for first_letter in first_letters:
        condition += first_letter + ' == 0'
        if i < len(first_letters) - 1:
            condition += ' or '
        i += 1
    f = ''
    if len(condition) > 0:
        f = 'lambda %s: False if %s else %s' % (parms, condition, body)
    else:
