...
def strategy_d(state):
    ...
    "Always double if possible and my score has reached goal, decline if opponent propose doubling and his score has reached the goal, always accept, hold at 20 pending."
    return ('double' if (double != 'double' and double < 2 and me+pending >= goal) else
            'decline' if (double == 'double' and you+pending >= goal) else 
            'accept' if double == 'double' else
