...
# game is played with a deck of cards in which each card is labeled
...
def superior(A, B=take5):
    ...
    print average_score(A)
    print average_score(B)
    return average_score(A) - average_score(B) > 1.5

def strategy(state):
    (_, yard, cards) = state
    hens_left = cards.count('H')
    foxes_left = cards.count('F')
    if foxes_left == 0:
        return 'wait'
    if hens_left == 0:
        return 'gather'
    ratio = float(foxes_left) / float(hens_left)
    if yard < 3 and ratio < 0.4:
        return 'wait'
    else:
        return 'gather'
