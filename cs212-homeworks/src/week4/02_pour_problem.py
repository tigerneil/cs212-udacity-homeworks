...
def more_pour_problem(capacities, goal, start=None):
    ...
    def successors(state):
        # fill
        result = dict((tuple([gl if ind!=index else capacities[index] for ind, gl in enumerate(state)]),
                      ('fill', index))
                    for index, glass in enumerate(state))
#        for index, glass in enumerate(state):
#            new_state = list(state)
#            new_state[index] = capacities[index]
#            result[tuple(new_state)] = ('fill', index)

        # empty
        result.update(dict((tuple([gl if ind!=index else 0 for ind, gl in enumerate(state)]),
                      ('empty', index))
                    for index, glass in enumerate(state)))
#        for index, glass in enumerate(state):
#            new_state = list(state)
#            new_state[index] = 0
#            result[tuple(new_state)] = ('empty', index)
        #pour
        for index_i, glass_i in enumerate(state):
            for index_j, glass_j in enumerate(state):
                new_state = list(state)
                if glass_j+glass_i <= capacities[index_j]:
                    new_state[index_j] = glass_j+glass_i
                    new_state[index_i] = 0
                    result[tuple(new_state)] = ('pour', index_i, index_j)
                else:
                    new_state[index_j] = capacities[index_j]
                    new_state[index_i] = glass_i - (capacities[index_j] - glass_j)
                    result[tuple(new_state)] = ('pour', index_i, index_j)
        return result;
