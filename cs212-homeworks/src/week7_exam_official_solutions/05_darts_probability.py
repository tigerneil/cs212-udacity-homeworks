...
# I define several collections; it is important to understand them:
# points: the set of all non-zero point values that can be scored
# doubles: the subset of points that can be scored in a double ring
# ordered_points: a list of points in the order we want to search over
...
def ring_outcome(target, miss):
    ...
    if target == 'DB': # misses tripled; can miss to SB or to S
        return [('DB', 1.0 - 3*miss), ('SB', miss), ('S', 2.*miss)]
...
test(test_darts2)
for i in range(52): print i, best_target(i/100.)