...
# Here is an alternative implementation of alltriples.
# It is less efficient for large word lists.

'''
def alltriples(words):
    """Return a list of (start, mid, end) formed by combining words x and y 
    with i letters of overlap."""
    return [(x[:-i], y[:i], y[i:])
               for x in words 
               for y in words if x is not y 
