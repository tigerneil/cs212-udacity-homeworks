...
# I wanted to make calls like p1(0) reasonably efficient.  Some choices I considered:
# (1) Passing the __name__ to eval: that would work, but would do unneeded work.
# (2) Looping through the coefs, summing terms: faster, but not fast enough.
# (3) Compile the arithmetic formula just once, then run the function: that's it!
# And to make things a little faster, I use Horner's rule, which stops me from
# having to evaluate exponents multiple times; I just add and multiply.

@memo
def polynomial(coefs):
    """Return a polynomial function with these attributes.  Memoized, so any
    two polynomials with the same coefficients will be identical polynomials."""
    # We build a function by evaluating a lambda expression in the empty environment.
...
class poly(object):
    ...
    def __init__(self, coefs):
        ...
        horner = ' + x * ('.join(map(str, coefs)) + ')'*(len(coefs)-1)
        self.fn = eval('lambda x: ' + horner, {})
    ...
    def __rmul__(self, p2): return mul(self, coerce_poly(p2)) # p * p2
    def __radd__(self, p2): return add(self, coerce_poly(p2)) # p + p2

    def __repr__(self):
        return '<poly ' + self.__name__ + '>'
...
def canonical(coefs):
    """Canonicalize coefs by dropping trailing zeros and converting to a tuple.
    Also allow a poly or a raw number as alternative inputs."""
    if isinstance(coefs, (int, float)): coefs = (coefs,)
    elif hasattr(coefs, 'coefs'): coefs = coefs.coefs
    while coefs[-1] == 0 and len(coefs) > 1:
        del coefs[-1]
    return tuple(coefs)
...
def Poly(formula): 
    "My cheating implementation -- I didn't bother to write a parser."
