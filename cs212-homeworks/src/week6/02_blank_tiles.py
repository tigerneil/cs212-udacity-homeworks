        ...
        if is_letter(sq):
            ...
            for L in hand:
                if L in possibilities:
                    ...
                    for L_BLANK in LETTERS:
                        if L_BLANK in possibilities:
                            add_suffixes(hand.replace('_', '', 1), pre+L_BLANK.lower(), start, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    if pre.upper() in PREFIXES:
        for L in hand:
            if L == '_':
                for L_BLANK in LETTERS:
                    find_prefixes(hand.replace('_', '', 1), pre+L_BLANK.lower(), results)
            else:
