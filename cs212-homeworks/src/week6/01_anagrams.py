...
def anagrams(phrase, shortest=2):
    ...
    result = set()
    find_anagrams(phrase, shortest, str_len_no_space(phrase), '', result)
    print result
    return result

def find_anagrams(letters, shortest, max_len, cur_anagram, result):
    words = find_words(letters)
    for word in words:
        if len(word) >= shortest:
            if str_len_no_space(cur_anagram + word) == max_len:
                anagram = cur_anagram + word
                anagram = sorted_anagram(anagram.strip())
                if not permutation_exists(anagram, result):
                    result.add(anagram)
            elif str_len_no_space(cur_anagram + word) < max_len:
                find_anagrams(removed(letters, word), shortest, max_len, cur_anagram + word + ' ', result)

def sorted_anagram(anagram):
    words = anagram.split()
    return ' '.join(sorted(words))

def permutation_exists(anagram, result):
    words = anagram.split()
    for an in result:
        an_words = an.split()
        return words in an_words

def str_len_no_space(string):
...
# FIXME: this file was provided by the Udacity, so could be used when submitting this code there.
# You should have your own file with words if you want to run this code in any other environment.
